            ...
            if self.session.cookies or self.cookies_string:
                ...
                if test_response.status_code == 200:
                    ...
                    if 'id.itmo.ru' in test_response.url or 'login' in test_response.url.lower():
                        ...
                        # Если мы на правильном URL и нет редиректа на авторизацию - считаем авторизованными
                        # (страница может быть SPA и загружать данные через JavaScript)
                        if 'id.itmo.ru' not in test_response.url and 'login' not in test_response.url.lower():
                            # Проверяем, что это не страница авторизации по содержимому
                            text_lower = test_response.text.lower()[:2000]
                            if 'login' not in text_lower and 'authorize' not in text_lower and 'войти' not in text_lower:
                                # Если страница содержит признаки личного кабинета или расписания - авторизованы
                                if 'личный кабинет' in text_lower or 'schedule' in test_response.url.lower() or 'расписание' in text_lower:
                                    self.is_authenticated = True
                                    logger.info("✅ Авторизация через куки успешна! (страница расписания загружена)")
                                    return True
                            
                            # Дополнительная проверка через парсинг HTML
                            soup = BeautifulSoup(test_response.text, 'html.parser')
                            # Ищем признаки страницы расписания (не страницы авторизации)
                            schedule_indicators = soup.find_all(['div', 'section'], class_=re.compile(r'schedule|lesson|class', re.I))
                            if schedule_indicators or 'schedule' in test_response.text.lower()[:1000]:
                                self.is_authenticated = True
                                logger.info("✅ Авторизация через куки успешна! (найдены элементы расписания)")
                                return True
                            else:
                                # Если не нашли элементы, но URL правильный и нет признаков авторизации - все равно считаем авторизованными
                                # (SPA приложения могут не иметь элементов в начальном HTML)
                                logger.info("ℹ️ Элементы расписания не найдены в HTML (возможно SPA), но URL правильный - считаем авторизованными")
                                self.is_authenticated = True
                                logger.info("✅ Авторизация через куки успешна!")
                                return True
                else:
