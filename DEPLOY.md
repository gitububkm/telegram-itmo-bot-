# Инструкции по деплою Telegram бота на Render

## Переменные окружения в Render Dashboard

Установите следующие переменные окружения в Render Dashboard:

### Обязательные переменные:
- `TELEGRAM_BOT_TOKEN` - токен вашего Telegram бота (получить у @BotFather)
- `RENDER_APP_NAME` - имя вашего приложения на Render (например: `telegram-itmo-bot`)
  - ⚠️ **ВАЖНО:** Это имя должно точно совпадать с именем вашего Web Service в Render Dashboard!
  - Если не установлено, код попытается использовать `RENDER_SERVICE_NAME` (автоматически устанавливается Render)

### Источник расписания (выберите один из вариантов):

**Вариант 1: Получение с my.itmo.ru (рекомендуется)**
- `ITMO_LOGIN` - ваш логин ITMO ID
- `ITMO_PASSWORD` - ваш пароль ITMO ID

**Вариант 2: Статическое расписание из JSON**
- `SCHEDULE_JSON` - JSON с расписанием занятий

### Опциональные переменные:
- `PORT` - порт для веб-сервера (по умолчанию: 10000)
- `WEBHOOK_URL` - полный URL webhook (если не установлен RENDER_APP_NAME)

## Настройка Render Web Service

1. **Build Command**: `pip install -r requirements.txt`
2. **Start Command**: `python main.py`
3. **Python Version**: 3.11.9 (указано в runtime.txt)

## Автоматическая установка Webhook

Бот автоматически установит webhook при запуске на URL:
`https://<RENDER_APP_NAME>.onrender.com/webhook`

## Проверка работы

После деплоя проверьте:
- Главная страница: `https://<RENDER_APP_NAME>.onrender.com/` - должна показать "Bot is running"
- Статус: `https://<RENDER_APP_NAME>.onrender.com/status` - JSON с информацией о боте
- Health check: `https://<RENDER_APP_NAME>.onrender.com/health` - проверка работоспособности
- Webhook info: `https://<RENDER_APP_NAME>.onrender.com/check-webhook` - проверка настроек webhook

## Диагностика проблем

### Сервис приостановлен ("This service has been suspended by its owner")

Если вы видите сообщение "This service has been suspended by its owner" при попытке открыть URL:

1. **Откройте Render Dashboard:**
   - Перейдите на https://dashboard.render.com
   - Найдите ваш Web Service `telegram-itmo-bot`

2. **Проверьте статус сервиса:**
   - Если сервис показывает статус "Suspended" или "Paused"
   - Нажмите кнопку **"Resume"** или **"Restart"** чтобы возобновить работу

3. **Если сервис был удален:**
   - Создайте новый Web Service
   - Подключите тот же репозиторий GitHub
   - Настройте переменные окружения заново
   - Build Command: `pip install -r requirements.txt`
   - Start Command: `python main.py`

4. **Проверьте лимиты Render Free Tier:**
   - Убедитесь, что не превышены лимиты бесплатного плана
   - Проверьте, что у вас есть доступ к аккаунту

### Бот не отвечает на сообщения

1. **Проверьте webhook URL:**
   - Откройте `https://<RENDER_APP_NAME>.onrender.com/check-webhook`
   - Убедитесь, что `webhook_url` правильный и совпадает с вашим Render URL
   - Проверьте, что нет ошибок в `last_error_message`

2. **Проверьте логи в Render Dashboard:**
   - Откройте Logs в Render Dashboard
   - Отправьте сообщение боту
   - Проверьте, появляются ли логи с `📨 Получен webhook update`

3. **Проверьте переменные окружения:**
   - `RENDER_APP_NAME` должно совпадать с именем вашего приложения в Render
   - `TELEGRAM_BOT_TOKEN` должен быть правильным токеном от @BotFather

4. **Проверьте статус бота:**
   - Откройте `https://<RENDER_APP_NAME>.onrender.com/status`
   - Убедитесь, что `bot_running: true` и `webhook_set: true`

## Логи

Все события логируются в консоль Render:
- Успешный запуск сервера
- Установка webhook
- Получение и обработка обновлений от Telegram

## Особенности Render Free Tier

- Приложение может "засыпать" после 15 минут неактивности
- При первом запросе после сна может потребоваться время на "пробуждение"
- Webhook автоматически переустанавливается при каждом запуске
- Сервис может быть автоматически приостановлен при длительном простое
- Для возобновления работы нажмите "Resume" в Render Dashboard